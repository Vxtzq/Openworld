...
class InfiniteTerrain(Entity):
    def __init__(self, **kwargs):
        ...
        self.biome1 = ["tree1","tree2","tree3","tree4","tree5","tree6"]
        self.entities = []
                        ...
                        for j in range(1,self.chunk_size):
                            if random.randint(0,100)>90:
                                ...
                                if heightmap[i*(self.chunk_size+1)+j] > 1 and heightmap[i*(self.chunk_size+1)+j-16]-1 < 6:
                                    Entity(model="grasstest",position=(i,heightmap[i*(self.chunk_size+1)+j-16]+.3,j),rotation=(0,random.randint(0,360),0),color=color.green,parent=new_chunk,double_sided=True)
                    for i in range(1,self.chunk_size):
                        for j in range(1,self.chunk_size):
                            if random.randint(0,100)>97:
                                
                                if heightmap[i*(self.chunk_size+1)+j] > 1 and heightmap[i*(self.chunk_size+1)+j-16]-1 < 7:
                                    Entity(model=random.choice(self.biome1),position=(i,heightmap[i*(self.chunk_size+1)+j-16]-1,j),rotation=(0,random.randint(0,360),0),color=color.green,parent=new_chunk,double_sided=True)
