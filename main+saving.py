...
worldname = 'New World' 
if not os.path.exists(worldname):
    os.makedirs(worldname)
class InfiniteTerrain(Entity):
    def __init__(self, **kwargs):
        ...
        self.height_scale = 8
        self.chunk_distance = 0
        self.tree_fade_distance = 20
        self.entities = []
        self.cube_distance = 30
        self.tree_positions = {}
        self.tree_seed = 420
        self.tree_threshold = 0.5
        self.tree_noise_scale = 10
    ...
    def update(self):
        self.update_chunks()
        self.update_trees()
        print(len(self.tree_positions))
        
        #self.camera.x += .3
    def get_tree_density(self, x, z):
        # Perlin noise settings for tree distribution
        scale = 50.0
        octaves = 4
        persistence = 0.5
        lacunarity = 2.0

        noise_value = pnoise2(x / scale, z / scale, octaves=octaves, persistence=persistence, lacunarity=lacunarity, base=self.seed)
        normalized_value = (noise_value + 1) / 2  # Normalize to range [0, 1]
    ...
    def generate_trees(self, chunk_x, chunk_z):
        random.seed(self.tree_seed + chunk_x * 73856093 + chunk_z * 19349663)
        tree_positions = []
        for i in range(self.chunk_size):
            for j in range(self.chunk_size):
                world_x = chunk_x * self.chunk_size + i
                world_z = chunk_z * self.chunk_size + j
                noise_value = pnoise2(world_x / self.tree_noise_scale, world_z / self.tree_noise_scale,
                                      octaves=self.octaves, persistence=self.persistence, lacunarity=self.lacunarity,
                                      repeatx=1024, repeaty=1024, base=self.tree_seed)
                if noise_value > self.tree_threshold:
                    y = 2
                    tree_positions.append((world_x, y, world_z))
        self.tree_positions[(chunk_x, chunk_z)] = tree_positions

    def update_trees(self):
        player_position = self.camera.position
        player_chunk_x = int(player_position.x // self.chunk_size)
        player_chunk_z = int(player_position.z // self.chunk_size)

        for (chunk_x, chunk_z), positions in self.tree_positions.items():
            for position in positions:
                if distance(player_position, position) < self.chunk_size * self.chunk_distance:
                    tree = Entity(model="tree", position=position)
                    self.entities.append(tree)

        for entity in self.entities:
            if distance(player_position, entity.position) > self.chunk_size * self.chunk_distance:
                destroy(entity)
