                    ...
                    for i in range(1,self.chunk_size):
                        for j in range(1,self.chunk_size):
                            if random.randint(0,100)>97:
                                if vertices[i*self.chunk_size+j][1] > 1 and vertices[i*self.chunk_size+j][1] < 9:
                                    average_y = self.get_average_height(i, j, vertices, self.chunk_size)
                                    Entity(model="tree",position=(i,average_y,j),shader=lit_with_shadows_shader,parent=new_chunk)
