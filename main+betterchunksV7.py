...
class InfiniteTerrain(Entity):
    def __init__(self, **kwargs):
        ...
        self.entities = []
        self.cube_distance = 5*16
        self.tree_threshold = 0.2
        self.tree_scale = 1
    ...
    def get_terrain_height(self, x, z):
        ...
        if chunk_key in self.chunks:
            ...
            # Assuming the vertices are stored in a flat list
            index = (local_z * (self.chunk_size + 1) + local_x)
            return chunk.model.vertices[index][1]  # Access the y-coordinate for height
        return 0  # Default height if chunk is not found

    def generate_trees(self):
        chunk_x = self.camera.x//self.chunk_size
        chunk_z = self.camera.z//self.chunk_size
        tree_positions = []
        for i in range(self.chunk_size):
            for j in range(self.chunk_size):
                world_x = chunk_x * self.chunk_size + i
                world_z = chunk_z * self.chunk_size + j
                tree_density = pnoise2(world_x / self.tree_scale, world_z / self.tree_scale, octaves=1, persistence=0.5, lacunarity=2.0, repeatx=1024, repeaty=1024, base=self.tree_seed)
                if tree_density > self.tree_threshold:
                    y = self.get_terrain_height(world_x, world_z)
    ...
    def update_trees(self):
        ...
        if (player_chunk_x, player_chunk_z) not in self.tree_positions:
            self.generate_trees(player_chunk_x, player_chunk_z)

        tree_positions = self.tree_positions.get((player_chunk_x, player_chunk_z), [])
        for pos in tree_positions:
            if len(self.entities) < 400 and distance(player_position, pos) <= self.cube_distance:
                tree = Entity(model="tree", position=pos)
