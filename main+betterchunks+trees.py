...
class InfiniteTerrain(Entity):
    def __init__(self, **kwargs):
        ...
        self.tree_distance = 0
        self.tree_fade_distance = 20
        self.tree_scale = 50
        self.tree_threshold = 0.1
    ...
    def generate_trees(self, chunk, chunk_x, chunk_z):
        chunk_size_plus_one = self.chunk_size + 1
        ...
        for i in range(chunk_size_plus_one):
            for j in range(chunk_size_plus_one):
                ...
                tree_noise = pnoise2(world_x / self.tree_scale, world_z / self.tree_scale, 
                                     octaves=self.octaves, persistence=self.persistence, lacunarity=self.lacunarity, 
                                     repeatx=1024, repeaty=1024, base=self.seed)

                if tree_noise > self.tree_threshold:
                    # Place a tree
                    
                
                    #if abs(self.camera.x-(chunk_x * self.chunk_size + i))<self.tree_fade_distance or abs(self.camera.z-(chunk_z * self.chunk_size + j))<self.tree_fade_distance:
                    tree = Entity(model='cube', color=color.brown, scale=(10, 10, 10))
                    tree_y = chunk.model.vertices[i * chunk_size_plus_one + j][1]  # Get the height of the terrain at this point
                    tree.position = (chunk_x * self.chunk_size + i, tree_y + 1, chunk_z * self.chunk_size + j)  # Place tree above the terrain
                    tree.parent = chunk  # Parent the tree to the chunk
    ...
    def update_chunks(self):
        ...
        for x in range(player_chunk_x - self.tree_distance, player_chunk_x + self.tree_distance + 1):
            for z in range(player_chunk_z - self.tree_distance, player_chunk_z + self.tree_distance + 1):
                if (x,z) not in self.trees:
                    self.generate_trees(self.chunks[(x, z)], x, z)
                    self.trees[(x,z)] = 1
    ...
    def remove_distant_trees(self):
        player_chunk_x = int(self.camera.x // self.chunk_size)
        player_chunk_z = int(self.camera.z // self.chunk_size)

        for (x, z), chunk in self.chunks.items():
            if abs(x - player_chunk_x) > self.tree_distance or abs(z - player_chunk_z) > self.tree_distance:
                for tree in chunk.children:
                    if tree.model == 'cube':  # Assuming trees are represented by cube models
                        tree.parent = None  # Remove the parent relationship
                        tree.eternal = False
                        destroy(tree)  # Disable the tree entity
