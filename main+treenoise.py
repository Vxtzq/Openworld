...
class InfiniteTerrain(Entity):
    ...
    def update_trees(self):
        ...
        # Spawn new trees
        # Spawn new trees based on Perlin noise
        for _ in range(10):
        # Introduce random offsets to x and z within a certain range
            x_offset = random.uniform(-self.cube_distance, self.cube_distance)
            z_offset = random.uniform(-self.cube_distance, self.cube_distance)

            x = int(player_position.x) + x_offset
            z = int(player_position.z) + z_offset

            y = self.get_terrain_height(x, z)
            tree_density = self.get_tree_density(x, z)

            if tree_density > random.uniform(1, 2):  # Adjust threshold as needed
                tree = Entity(model="tree", position=(x, y, z))
