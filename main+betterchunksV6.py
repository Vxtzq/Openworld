...
class InfiniteTerrain(Entity):
    ...
    def get_terrain_height(self, x, z):
        ...
        if isinstance(mesh, Mesh):
            idx = local_z * (self.chunk_size + 1) + local_x
            if idx < len(mesh.vertices):
                return mesh.vertices[idx][1]

        return 0

    def generate_trees(self, chunk_x, chunk_z):
        random.seed((chunk_x, chunk_z, self.tree_seed))  # Set the seed for reproducibility
        player_chunk_x = int(self.camera.x // self.chunk_size)
        player_chunk_z = int(self.camera.z // self.chunk_size)
        for _ in range(10):
            x = random.uniform(player_chunk_x * self.chunk_size, (player_chunk_x + 1) * self.chunk_size)
            z = random.uniform(player_chunk_z * self.chunk_size, (player_chunk_z + 1) * self.chunk_size)
